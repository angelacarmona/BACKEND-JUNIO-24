...
const checkProduct = async (value, { req }) => {
  try {
    ...
    const fechaVisibilidad = new Date(req.body.visibleUntil)
    const fechaActual = new Date()
    if (fechaVisibilidad.getTime() < fechaActual.getTime()) {
      return Promise.reject(new Error('visibleUntil date is before current date'))
    } else if (product.availability === false && product.visibleUntil) {
      return Promise.reject(new Error('cannot be unavailable and have visibleUntil date at the same time'))
    } else { return Promise.resolve() }
  } catch (err) {
    return Promise.reject(new Error(err))
  }
